...
export interface ClientOptions {
	 ...
	 *
	 * Defaults to process.env['TRIGLIT_BASE_URL'].
	 */
	 ...
	 * Set the log level.
	 *
	 * Defaults to process.env['TRIGLIT_LOG'] or 'warn' if it isn't set.
	 */
...
export class Triglit {
	 ...
	 */
	constructor({
		baseURL = readEnv('TRIGLIT_BASE_URL'),
		apiKey = readEnv('TRIGLIT_API_KEY'),
		...opts
	}: ClientOptions = {}) {
		if (apiKey === undefined) {
			throw new Errors.TriglitError(
				"The TRIGLIT_API_KEY environment variable is missing or empty; either provide it, or instantiate the Triglit client with an apiKey option, like new Triglit({ apiKey: 'My API Key' }).",
			);
		}
		...
		this.timeout = options.timeout ?? Triglit.DEFAULT_TIMEOUT /* 1 minute */;
		...
		this.logLevel =
			...
			parseLogLevel(readEnv('TRIGLIT_LOG'), "process.env['TRIGLIT_LOG']", this) ??
			defaultLogLevel;
	...
	static TriglitError = Errors.TriglitError;
